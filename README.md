...
to pre-build the tabs — the script creates `Transactions`, `Transfers`, and
`Events` automatically the first time it runs, with headers. If you'd rather
create them yourself first:

- **Transactions**: Timestamp, Date, Type, Person, Amount, Event, Category, Notes
- **Transfers**: Timestamp, Date, From, To, Amount, Notes
- **Events**: Timestamp, Event, Notes

## Editing a logged entry

The Overview page has a Transaction & Transfer Log section at the bottom.
Click **Edit** on any row to correct a wrong amount, date, person, category,
or event, then **Save**. This rewrites that exact row in the Sheet — nothing
is duplicated. `Code.gs` matches rows by their actual sheet row number
(sent to the browser as `_row`), so don't manually reorder or delete rows in
the Sheet while people are mid-edit in the app.

## Creating a new event

On the Log page, the Event dropdown (for both Cash In and Cash Out) includes
a **"+ New event…"** option at the bottom. Pick it, type a name, and it's
saved to the `Events` tab and used for that entry — from then on, everyone
else sees it in their own Event dropdown (after they reload the page) and
can log against it without retyping anything.
